...
void solve() {
  int n, k;
  cin >> n >> k;
  vector<vi32> books(10);
  vector<vi32> sums(10, vi32(k + 1));
  rep(i, n) {
    int c, g;
    cin >> c >> g;
    g--;
  ...
  rep(i, 10) sort(all(books[i])), reverse(all(books[i]));
  rep(i, 10) {
    rep(j, books[i].size()) {
      sums[i][j + 1] = books[i][j];
      sums[i][j + 1] += sums[i][j] + j * 2;
    }
    repic(j, books[i].size() + 1, k) sums[i][j] = -1;
  }
  vector<vi32> dp(11, vi32(k + 1));
  rep(i, 10) {
    repc(j, k) {
      int ma = 0;
      repc(l, j) {
        if (sums[i][l] == -1) break;
        amax(ma, sums[i][l] + dp[i][j - l]);
      }
      dp[i + 1][j] = ma;
    }
  }
