...
void solve() {
  int n, m, l;
  cin >> n >> m >> l;
  vector<vi64> dist(n, vi64(n, 1e18));
  rep(i, n) dist[i][i] = 0;
  rep(i, m) {
    int a, b, c;
    cin >> a >> b >> c;
  ...
  rep(k, n) rep(i, n) rep(j, n) amin(dist[i][j], dist[i][k] + dist[k][j]);

  vector<vi32> cnt(n, vi32(n, 1e9));
  rep(i, n) cnt[i][i] = 0;
  rep(i, n) rep(j, n) {
    if (i == j) continue;
    if (dist[i][j] <= l) cnt[i][j] = cnt[j][i] = 1;
  }
  rep(k, n) rep(i, n) rep(j, n) amin(cnt[i][j], cnt[i][k] + cnt[k][j]);
  ...
  rep(_, q) {
    ...
    cin >> s >> t;
    s--, t--;
    cout << (cnt[s][t] == 1e9 ? -1 : cnt[s][t] - 1) << endl;
  }
}
