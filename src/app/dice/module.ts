...
export function reverse_half_sphere(r: number, color = [1, 1, 1], c = 16) {
...
function face(points: { x: number, y: number }[], edge = 10, size = 1, color = [0, 0, 0]) {
  const E = edge;
  const M = size;
  const C = color;
  ...
  const ys: number[] = [];
  {
    ...
    const xset = xa.reduce((a, c) => a.add(c), new Set<number>());
    const yset = ya.reduce((a, c) => a.add(c), new Set<number>());
