import { Vec3, triangle_contains_point,
         intersection_of_plane_and_line } from "./geometry.js";
...
expect.extend({
  toEqualVec3(received, expected) {
    ...
    if (pass) {
      ...
      return {
        message: () =>
          `expected Vec3(${expected.primitive()}) not to be Vec3(${received.primitive()})`,
        pass
      };
    }
  }
...
describe("Vec3", () => {
  ...
  test("equals()", () => {
    expect(V).toEqualVec3(Vec3(1, 2, 3));
    expect(V).not.toEqualVec3(Vec3(1, 2, 4));
  });

  test("add()", () => {
    expect(V.add(U)).toEqualVec3(Vec3(5, 7, 9));
  });

  test("sub()", () => {
    expect(U.sub(V)).toEqualVec3(Vec3(3, 3, 3));
  });

  test("inverse()", () => {
    expect(V.inverse()).toEqualVec3(Vec3(-1, -2, -3));
  });
  ...
  test("cross()", () => {
    {
      const A = Vec3(1, -4, -2);
      const B = Vec3(2,  5,  1);
      expect(A.cross(B)).toEqualVec3(Vec3( 6, -5,  13));
      expect(B.cross(A)).toEqualVec3(Vec3(-6,  5, -13));
    }
    {
      const A = Vec3(2, 4, -3);
      const B = Vec3(3, 8, -5);
      expect(A.cross(B)).toEqualVec3(Vec3( 4,  1,  4));
      expect(B.cross(A)).toEqualVec3(Vec3(-4, -1, -4));
    }
  });

  test("pure", () => {
    const v1 = Vec3(1, 2, 3);
    const v2 = v1.add(Vec3(4, 5, 6));
    const v3 = v2.sub(Vec3(7, 8, 9));
    const v4 = v1.cross(v3);
    const v5 = Vec3(0, 0, 10000);
    const v6 = v5.normalize();
    const v7 = v6.inverse();
    const v8 = v7.scale(100);
    const d1 = v1.dot(v4);
    expect(v1).toEqualVec3(Vec3(1, 2, 3));
    expect(v2).toEqualVec3(Vec3(5, 7, 9));
    expect(v3).toEqualVec3(Vec3(-2, -1, 0));
    expect(v4).toEqualVec3(Vec3(3, -6, 3));
    expect(v5).toEqualVec3(Vec3(0, 0, 10000));
    expect(v6).toEqualVec3(Vec3(0, 0, 1));
    expect(v7).toEqualVec3(Vec3(0, 0, -1));
    expect(v8).toEqualVec3(Vec3(0, 0, -100));
    expect(d1).toBe(0);
  });
});
...
describe("triangle_contains_point()", () => {
  ...
  test("bugfix", () => {
    const A = Vec3(-50, 5, 0);
    const B = Vec3(-100, 3.7187728881835938, 0);
    const C = Vec3(-50, 5, 50);
    const Q = Vec3(443.86936557241273, 17.655176417338865, 0);
