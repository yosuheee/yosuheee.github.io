...
describe("mathematics", () => {
  ...
  test("simulate shader program", () => {
    ...
    {
      const r = Mat4.rotate(V3(0, 1, 0), -90).translate(1, 2, 3);
      const received = [];
      for (const { n } of (it(p))) {
        received.push(n.dot(reverse_light_direction.transformMat4(r, 0)));
      }
      for (let i = 0; i < expected.length; i++) {
        expect(received[i]).toBeCloseTo(expected[i]);
      }
    }
    {
      ...
      const received = [];
      for (const { n } of (it(p))) {
        received.push(n.dot(reverse_light_direction.transformMat4(r.invert(), 0)));
      }
      for (let i = 0; i < expected.length; i++) {
        expect(received[i]).toBeCloseTo(expected[i]);
      }
