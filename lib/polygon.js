...
export function sphere(r, c = [1, 1, 1], row = 64, col = 64) {
  const data = [], index = [];
  for (let i = 0; i <= row; i++) {
    const langle = Math.PI / 180 * 180 / row * i;
    const rad = r * Math.sin(langle);
    const y = r * Math.cos(langle);
    for (let j = 0; j < col; j++) {
      const sangle = Math.PI / 180 * 360 / col * j;
  ...
  for (let i = 0; i < row; i++) {
    for (let j = 0; j < col; j++) {
      const s1 = i * col + j;
      const s2 = s1 + 1 === (i + 1) * col ? i * col : s1 + 1;
      const t1 = (i + 1) * col + j;
      const t2 = t1 + 1 === (i + 2) * col ? (i + 1) * col : t1 + 1;
      index.push([s1, s2, t2]);
      index.push([s1, t2, t1]);
    }
  }
