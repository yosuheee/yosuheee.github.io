...
export class Polygon {
  ...
  *triangles() {
  ...
  flatten(eps = 0.001) {
    ...
    for (let i = 0; i < n; i++)
    for (let j = 0; j < i; j++) {
      const l = this.data[i].position.sub(this.data[j].position).length();
      if (l <= eps) {
        uf.unite(i, j);
      }
    }
    ...
    const index = this.index.map(idx => range(3).map(i => di[uf.root(idx[i])]));
    return new Polygon(data, index, this.tridata);
  }
}
