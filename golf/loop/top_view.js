...
export function top_view_loop(Game, prev = new Date().getTime()) {
  ...
  const lateral_direction = p => {
    ...
    const a = V3(1, 0 ,0).rotate(V3(0, 1, 0), Game.hit.angle);
    const b = Game.camera.center.sub(p);
    const d = xz_distance(p, Game.camera.center) * (a.dot(b) >= 0 ? 1 : -1);
    Game.camera.center = V3(d, 0, 0).rotate(V3(0, 1, 0), Game.hit.angle).add(p);
    const y = Game.camera.position.y - Game.camera.center.y;
    Game.camera.position = Game.camera.center.add(V3(0, y, 0));
  };
  ...
  if (Game.top.up_key) {
    const move = V3((now - prev) / 10, 0, 0).rotate(V3(0, 1, 0), Game.hit.angle);
  ...
  if (Game.top.up_shift_key) {
    ...
    const temp = Game.camera.position.sub(V3(0, move, 0));
    const p = Game.world.positions.slice(-1)[0];
  ...
  if (Game.top.down_shift_key) {
    const move = (now - prev) / 5;
    const temp = Game.camera.position.add(V3(0, move, 0));
    const p = Game.world.positions.slice(-1)[0];
